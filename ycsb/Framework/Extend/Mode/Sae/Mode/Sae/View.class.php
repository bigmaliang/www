...
// +----------------------------------------------------------------------
// $Id: View.class.php 2587 2012-01-13 16:04:28Z zuojiazi.cn@gmail.com $
 ...
 * @subpackage  Core
 * @author liu21st <liu21st@gmail.com>
 * @version  $Id: View.class.php 2587 2012-01-13 16:04:28Z zuojiazi.cn@gmail.com $
 +------------------------------------------------------------------------------
 */
class View extends Think{
    ...
    public function fetch($templateFile='',$charset='',$contentType='',$display=false) {
        ...
        //sae读取模版内容
        $tmplCacheFile = "Tpl/" . md5($templateFile) . C('TMPL_CACHFILE_SUFFIX');
        ...
        }elseif('think' == $engine && $content !== false && $this->checkCache($templateFile, $cache->mtime)) {//sae环境下的模版缓存判断
            // 如果是Think模板引擎并且缓存有效 分解变量并载入模板缓存
            extract($this->tVar, EXTR_OVERWRITE);
            //载入模版缓存文件
            eval('?>'.$content);//sae下，载入缓存文件
        }else{
     ...
     * @return boolen
     +----------------------------------------------------------
     */
    protected function checkCache($tmplTemplateFile, $mtime) {//sae下判断模版缓存时间，增加mtime参数
