...
// +----------------------------------------------------------------------
// $Id: functions.php 2504 2011-12-28 07:35:29Z liu21st $
 ...
 * @package  Common
 * @author   liu21st <liu21st@gmail.com>
 * @version  $Id: functions.php 2504 2011-12-28 07:35:29Z liu21st $
  +------------------------------------------------------------------------------
 */
// 设置和获取统计数据
...
function import($class, $baseUrl = '', $ext='.class.php') {
    ...
    if (substr($baseUrl, -1) != "/")
        $baseUrl .= "/";
    //如果存在sae专用类，则加载
    $classfile = is_file($baseUrl . $class . "_sae" . $ext) ? $baseUrl . $class . "_sae" . $ext : $baseUrl . $class . $ext;
...
//以下是Sae函数
/**
 * 分析地址，取文件名前的目录为seastorage的domain。
 * 如果指定了domain，去domain之后为文件名。
 */
function sae_parse_path($path, $domain='') {
    if (empty($domain)) {
        $arr = explode("/", $path);
        $c=count($arr);
        return array(
