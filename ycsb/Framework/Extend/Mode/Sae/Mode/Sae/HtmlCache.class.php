...
// +----------------------------------------------------------------------
// $Id: HtmlCache.class.php 2504 2011-12-28 07:35:29Z liu21st $
 ...
 * @subpackage  Util
 * @author liu21st <liu21st@gmail.com>
 * @version  $Id: HtmlCache.class.php 2504 2011-12-28 07:35:29Z liu21st $
 +------------------------------------------------------------------------------
 */
class HtmlCache extends Think
{
        ...
        // 分析当前的静态规则
         ...
         if(!empty($htmls)) {
            ...
            if(!empty($html)) {
                ...
                // 当前缓存文件
                define('HTML_FILE_NAME','html/' . $rule.C('HTML_FILE_SUFFIX'));//sae定义静态页面地址。
                return true;
            }
        }
        // 无需缓存
    ...
    static function readHTMLCache() {
        if(self::requireHtmlCache() && self::checkHTMLCache(HTML_FILE_NAME,self::$cacheTime)) { //静态页面有效
            ...
            if(C('HTML_READ_TYPE')==1) {
                // 重定向到静态页面
                redirect($s->getUrl(C('SAE_THINK_DOMAIN'), HTML_FILE_NAME));
            }else {
                // 读取静态页面输出
                readfile($s->getUrl(C('SAE_THINK_DOMAIN'), HTML_FILE_NAME));
                exit();
            }
        }
        return ;
    ...
    static public function writeHTMLCache($content) {
        if(self::$requireCache) {
            ...
            //sae下写入操作
            $s=Think::instance('SaeStorage');
            if( false === $s->write(C('SAE_THINK_DOMAIN'),HTML_FILE_NAME , $content ))
                throw_exception(L('_CACHE_WRITE_ERROR_').':'.HTML_FILE_NAME.'请检查是否已建立domin'.C('SAE_THINK_DOMAIN').'为的storage');
        }
        return ;
    }
    ...
    static public function checkHTMLCache($cacheFile='',$cacheTime='') {
        //sae下，判断缓存
        $s=Think::instance('SaeStorage');
        $attr=$s->getAttr(C('SAE_THINK_DOMAIN'), $cacheFile);
        if($s->errno()!=0){
            return false;
        }elseif (filemtime(C('TMPL_FILE_NAME')) > $attr['datetime']) {
            // 模板文件如果更新静态文件需要更新
            return false;
        }elseif(!is_numeric($cacheTime) && function_exists($cacheTime)){
            return $cacheTime($cacheFile);
        }elseif ($cacheTime != -1 && time() > $attr['datetime']+$cacheTime) {
            // 文件是否在有效期
            return false;
        }
        //静态文件有效
