...
// +----------------------------------------------------------------------
// $Id: ThinkTemplate.class.php 2504 2011-12-28 07:35:29Z liu21st $
 ...
 * @subpackage  Template
 * @author liu21st <liu21st@gmail.com>
 * @version  $Id: ThinkTemplate.class.php 2504 2011-12-28 07:35:29Z liu21st $
 +------------------------------------------------------------------------------
 */
...
{//类定义开始
    ...
    public function load($templateFile,$templateVar,$charset) {
        ...
        //载入模版缓存文件
        eval('?>'.$templateCacheFile);//sae载入缓存。
    }
    ...
    public function loadTemplate ($tmplTemplateFile='') {
        ...
        //根据模版文件名定位缓存文件
        $tmplCacheFile = "Tpl/".md5($tmplTemplateFile).$this->config['cache_suffix'];//sae下缓存命名
       ...
       //     mk_dir($this->config['cache_path']);
        ...
        if( false === $cache->set($tmplCacheFile,trim($tmplContent)))//sae环境下写入缓存
            throw_exception(L('_CACHE_WRITE_ERROR_').':'.$tmplCacheFile);
        return $tmplContent;// sae下返回内容，不是地址。
    }
