...
// +----------------------------------------------------------------------
// $Id: SaeTaskQueue.class.php 2504 2011-12-28 07:35:29Z liu21st $
/**
*任务列队
*本地环境暂时需要支持curl才行。
*/
	...
	public function addTask($tasks,$postdata=null,$prior=false,$options=array()){
		if ( is_string($tasks) ) {
			if ( !filter_var($tasks, FILTER_VALIDATE_URL) ) {
				...
				$this->errmsg = L("_SAE_ERRPARAMTER_");
				return false;
			}
	...
	public function push(){
		...
		foreach($this->queue as $k=>$v){  	
			...
			if(empty($info['http_code'])) {
				...
				$this->errmsg = L("_SAE_TASKQUEUE_SERVICE_FAULT_");
				return false;
			} else if($info['http_code'] != 200) {
				$this->errno = SAE_ErrInternal;
				$this->errmsg = L("_SAE_TASKQUEUE_SERVICE_ERROR_");
				return false;
			} else {
				if($info['size_download'] == 0) { // get MailError header
					$this->errno = SAE_ErrUnknown;
					$this->errmsg = L("_SAE_UNKNOWN_ERROR_");
					return false;
				} 
			} 
		}
